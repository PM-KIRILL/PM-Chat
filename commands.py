...
class Commands(commands.Cog, name="Модуль команд"):
    ...
    def get_stats(self):
        if config.use_query == True:
            ...
            stats["protocol_version"] = "Неизвестно"
        else:
    ...
    #команда для вывода списка игроков
    ...
    #команда для получения различной статистики сервера
    ...
    #команда для выполнения команды в mc
    ...
    async def run_cmd(self, ctx):
        ...
        try:
            ...
            print("Попытка повторного подключения к серверу RCON...")
            try:
                self.client.rcon.reconnect()
                response = self.client.rcon.send_cmd(cmd)
                print("Повторное подключение успешно!")
            except ConnectionRefusedError:
                response = "Ошибка: Подключение к серверу RCON было отклонено. Сервер выключен/запускается или RCON не включен."

        if response == "":
            response = "Команда была успешно выполнена, но ответ от сервера был пустым."
    ...
    #команда для отображения серверного motd
